        ...
        apiProvider.request(target: .search(query: query, page: page))
            .map { (response: SearchRepositoryResponse) -> [RepositoryData] in
                return response.items.map { item in
                    RepositoryData(id: item.id, name: item.name, owner: Owner(login: item.owner.login, avatarURL: item.owner.avatarURL), description: item.description, stargazersCount: item.stargazersCount, forksCount: item.forksCount, openIssuesCount: item.openIssuesCount)
                }
            }
            .eraseToAnyPublisher()
    }
